...
tauronPeSeptember = read.csv('TAURONPE.prn')
...
phases2 = data.frame(openFixings$DATE, openFixings$AMOUNT/septemberSum, duringDayTrading$AMOUNT/septemberSum, closeFixings$AMOUNT/septemberSum)
colnames(phases2) = c("date", "open", "during_day", "close")

ggplot(phases2, aes(date)) + geom_line(aes(y = open, color = "open fixing")) +
  geom_line(aes(y = close, color = "close fixing")) +
  geom_line(aes(y = during_day, color = "during day trading")) +
  labs(title = "Distribution of transactions in days depending on stock phase", x = "Data", y = "Percentage share of transactions")

#pudełkowy
phases = data.frame(amount = c(openFixings$AMOUNT/septemberSum, duringDayTrading$AMOUNT/septemberSum, closeFixings$AMOUNT/septemberSum),
...
ggboxplot(phases, x = "phase", y = "amount", color = "phase", palette = "npg") + labs(title="Distribution of transacrions in days depending onn the trading phase", x="Phase", y="Percentage share in transactions")
